...
function MobileForm(props) {
  const { rwd, globalData, globalDataDispatch, modalConfig, modalConfigDispatch, registor, registorDispatch, globalFunction } = React.useContext(DataContext)
  ...
  return (
    <div className={`mobile-form ${rwd.rwdMode}`}>
      <div className="form-frame">
        <div className="posts-header" onClick={() => { modalConfigDispatch({ index: -1 }) }}>
          <div className="back-button" onClick={() => { modalConfigDispatch({ ...modalConfig, openForm: false }) }}><FontAwesomeIcon icon={faArrowLeft} /></div><div className="posts-header-text">報名資訊</div>
        </div>
        <div className='form-content'>
          ...
          <div className="items" style={{ backgroundImage: `url("${BG}")` }}>
            <OurAbout />
            ...
            {globalData.form.map((item, k) => {
              return <div key={k} className="form-item">
                {item.type === "textarea" ? <textarea className={`${item.isTip ? "tip" : ""}`} data-key={item.key} placeholder={item.placeHolder} defaultValue={registor[item.key]} onKeyUp={keyUp} ref={refMap[item.key]} /> :
                  <>
                    <div className={`title ${item.required ? "required" : ""}`} id={`key${item.key}`}>{item.title}</div>
                    <div className="value">
                      {item.type === "text" && <input className={`${item.isTip ? "tip" : ""}`} tpye="text" data-key={item.key} defaultValue={registor[item.key]} onKeyUp={keyUp} ref={refMap[item.key]} />}
                      ...
                      {item.type === "textarea" && <textarea className={`${item.isTip ? "tip" : ""}`} data-key={item.key} defaultValue={registor[item.key]} onKeyUp={keyUp} ref={refMap[item.key]} />}
                    </div>
                  </>}
              </div>
            })}
          ...
          <div className={`success-mask ${(globalData.isSendSuccess || globalFunction.getIsSuccess()) && "show"}`}>
            {/* ${(globalData.isSendSuccess || globalFunction.getIsSuccess()) && "show"} */}
            <div className="success-mask-background"></div>
            <div className="success-mask-content">
              <div className="success-info">
                <div className='title'>報名成功<br />期待2023/09/16相會</div>
